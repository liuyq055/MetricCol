...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Keys in the dictionary are either strings or tuples:
            ...
            ("color", <frame_id>, <d4v1_scale>)          for raw colour images,
            ("color_aug", <frame_id>, <d4v1_scale>)      for augmented colour images,
            ("K", d4v1_scale) or ("inv_K", d4v1_scale)        for camera intrinsics,
        ...
        <d4v1_scale> is an integer representing the d4v1_scale of the image relative to the fullsize image:
        ...
        if self.load_depth:
            ...
            # print('gt', depth_gt.max(), depth_gt.min())
            inputs["depth_gt"] = np.expand_dims(depth_gt, 0)
            inputs["depth_gt"] = torch.from_numpy(inputs["depth_gt"].astype(np.float32))

        teacher = self.get_teacher(folder,frame_index,side,do_flip)
        if teacher is not None:
            # print('teacher', teacher.max(), teacher.min())
            inputs['teacher'] = np.expand_dims(teacher,0)
            inputs["teacher"] = torch.from_numpy(inputs['teacher'].astype(np.float32))
